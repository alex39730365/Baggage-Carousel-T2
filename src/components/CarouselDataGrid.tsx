...
/** `KE714 / NRT`, `KE 712` 등 본편 KE 판별 — 순환 import 회피용 사본 */
const isMainlineKeFlightLocal = (flight: string): boolean => {
  const head =
    flight
      .trim()
      .split(/\s*\/\s*/)[0]
...
export function CarouselDataGrid({
  ...
  return (
    <table className="w-full min-w-[1320px] table-fixed border-collapse border-slate-800 text-[10px] text-slate-800 sm:min-w-0 sm:text-[9px] lg:text-[10px]">
      <thead
                          ...
                          return (
                            <article
                              ...
                              data-clicked-ke={
                                highlighted &&
                                kePinkHighlight &&
                                isMainlineKeFlightLocal(item.flight)
                                  ? "true"
                                  : undefined
                              }
                              role="button"
